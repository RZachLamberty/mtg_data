...
def get_inventory(url=_URL, owner='ndlambo', pagelength=500):
    ...
    # useful lookup dict
    mtgjson = {
        (card.get('name'), card.get('setname')): card
...
class TappedoutDeck(decks.Deck):
    ...
    but will also have a to-specific dataframe of deck information as self.df (if
...
def binder_summary(url=_URL, owner='ndlambo', bulkthresh=0.30, mainthresh=1.00):
    ...
    keepkeys = [
        'name', 'qty', 'foil', 'px', 'tla', 'type', 'tcg-foil-price',
    ...
    inventory.loc[:, 'set'] = inventory['set'].apply(
        lambda s: s if isinstance(s, str) else s[0]
    )
    ...
    inventory.power = inventory.power.cat.reorder_categories(
        orderedpow, ordered=True
    )
    inventory.toughness = inventory.toughness.cat.reorder_categories(
        orderedtuf, ordered=True
    )
    ...
    foilpx = inventory.loc[usefoilpx, 'tcg-foil-price'].str.replace(',', '.').astype(float)
    inventory.loc[usefoilpx, 'price'] = foilpx

    # subset based on whether or not they meet my thresholds
    inventory.loc[:, 'card_value'] = _pd.cut(
        inventory.price,
        right=False,
    ...
    orderedcats = sorted(
        inventory.colorstr.cat.categories,
        key=color_category_order
    )
    inventory.colorstr = inventory.colorstr.cat.reorder_categories(
        orderedcats, ordered=True
    )

    # ditto for type
    inventory.loc[:, 'mytype'] = inventory.type.str.replace('\u2014', '-')
    inventory.loc[:, 'mytype'] = inventory.mytype.str.replace(' - ', '|')
    inventory.loc[:, 'mytype'] = inventory.mytype.str.extract('([^|]+)', expand=False)
    ...
    orderedtypes = sorted(
        inventory.mytype.cat.categories, key=type_category_order
    )
    inventory.mytype = inventory.mytype.cat.reorder_categories(
        orderedtypes, ordered=True
    )
    ...
    inventory = inventory.sort_values(
        by=[
            'card_value', 'is_land', 'colorstr', 'mytype', 'cmc', 'power',
            'toughness', 'name', 'foil'
        ]
    )
...
def binder_df_to_pagelists(inventory):
    ...
    """
    return [
        '{} ({}, {} of {})'.format(row['name'], row.type, i + 1, row.qty)
